...
describe Building do
  ...
  context "providing info to car drivers" do
  ...
  context "avoiding crashes" do
    it "should not let two cars park at the same spot" do
      building.take_spot! :floor => 1, :row => 0, :spot => 5
      expect {building.take_spot! :floor => 1, :row => 0, :spot => 5}.to raise_error
    end
  end
  ...
  context "following the laws of physics" do
    it "should not let cars leave a parking spot before they're actually parked there" do
      expect {building.release_spot! :floor => 1, :row => 0, :spot => 5}.to raise_error
    end
  end
